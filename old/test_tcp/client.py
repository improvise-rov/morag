...
class ConnectThread(t.Thread):
    ...
    def run(self):
        ...
        print(f"Connecting on {tcp_ip}:{tcp_port}")
        self.socket.connect((tcp_ip, tcp_port))

        while True:
            time.sleep(interval)
            if (shouldConnect):
                pulledData = ConnectTo(self.socket) ## connect

    def kill(self):
        self.socket.close()

def FetchDict(s: s.socket) -> dict:
    SendReq(0, s)
...
def SendReq(id: int, s: s.socket) -> None:
    s.sendall(bytes(chr(id), "UTF-8"))
...
## ARDUINO REMOTE CONTROL TCP PACKETS
def moveStepper_Up(s: s.socket):
    SendReq(1, s)

def moveStepper_Down(s: s.socket):
    SendReq(2, s)
...
if __name__ == "__main__":
    ...
    while not window_should_close(): # x = TIME, y = DEPTH
        ...
        if (is_key_pressed(KeyboardKey.KEY_UP)): 
            moveStepper_Up(cl.socket)

        if (is_key_pressed(KeyboardKey.KEY_DOWN)): # im yelling timber
            moveStepper_Down(cl.socket)
        ...
        # Graph
        addPlottedPoint(int(pulledData['time']), round(max(0, float(pulledData['depth'])), 2))
